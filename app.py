...
if st.button("Search Jobs"):
    if not query:
        ...
        with st.spinner("Searching..."):
            ...
            if not jobs:
                st.warning("⚠️ Could not retrieve jobs. Please check your connection or try a different role.")
            else:
                for job in jobs:
                    st.subheader(job.get("title", "Job Opening"))
                    st.write(f"Company: {job.get('company')}")
                    st.write("---")
# End of block - Ensure no hanging code follows